...
const Custom = ({
  ...
  useEffect(() => {
    networkTokens && !defaultValue && setValue(networkTokens[0]?.symbol);
  }, [networkTokens]);
  ...
  return (
    <div className={classesCustom["dropdown-wrapper"]}>
      ...
      {isOptionsOpen && (
        <div className={classesIndex["dropdown-options-container"]}>
          <input
            onChange={(e) => addToken(e.target.value)}
            onPaste={(e) => addToken(e.target.value)}
            className={classesIndex["dropdown-tokens-input"]}
            type="text"
          />
          {networkTokens?.map((option) => (
            <div
              key={`--option-${option.address}`}
